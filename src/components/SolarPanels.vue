<template>
    <div class="grid gap-10 grid-cols-2 grid-rows-2 h-full" v-if="state.solarData.realKpi">
        <div class="space-y-4 bg-slate-800 rounded-lg p-10 flex items-center justify-between flex-col w-full h-full">
            ...
            <div><span class="text-[8vw] leading-none outline-text">{{state.solarData.realKpi.realTimePower}}</span><span class="text-[2vw] text-white">kW</span></div>
            <div class="text-[2vw] text-slate-500">Real-time Forbrug</div>
        </div>
        <div class="space-y-4 bg-slate-800 rounded-lg p-10 flex items-center justify-between flex-col w-full h-full">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-16 h-16">
                <path stroke-linecap="round" stroke-linejoin="round" d="M12 3v2.25m6.364.386l-1.591 1.591M21 12h-2.25m-.386 6.364l-1.591-1.591M12 18.75V21m-4.773-4.227l-1.591 1.591M5.25 12H3m4.227-4.773L5.636 5.636M15.75 12a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0z" />
            </svg>
            <div><span class="text-[8vw] leading-none outline-text">{{state.solarData.realKpi.dailyEnergy}}</span><span class="text-[2vw] text-white">kWh</span></div>
        ...
        <div class="space-y-4 bg-slate-800 rounded-lg p-10 flex items-center justify-between flex-col w-full h-full">
            ...
            <div><span class="text-[8vw] leading-none outline-text">{{parseFloat(state.solarData.socialContribution.co2ReductionByYear / 1000).toFixed(2)}}</span><span class="text-[2vw] text-white">tons</span></div>
        ...
        <div class="space-y-4 bg-slate-800 rounded-lg p-10 flex items-center justify-between flex-col w-full h-full">
            ...
            <span class="text-[8vw] leading-none outline-text">{{state.solarData.socialContribution.equivalentTreePlanting}}</span>
