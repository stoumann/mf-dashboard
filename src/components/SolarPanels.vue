<template>
    <div class="grid gap-20 grid-cols-3 grid-rows-2 h-full" v-if="state.solarData.realKpi">
        <div class="space-y-4 bg-black rounded-lg p-10 flex items-center justify-between flex-col w-full h-full">
            ...
            <div><span class="text-[7vw] leading-none outline-text">{{state.solarData.realKpi.realTimePower}}</span><span class="text-[2vw] text-zinc-600">kW</span></div>
        ...
        <div class="space-y-4 bg-black rounded-lg p-10 flex items-center justify-between flex-col w-full h-full">
            ...
            <div><span class="text-[7vw] leading-none outline-text">{{state.solarData.realKpi.dailyEnergy}}</span><span class="text-[2vw] text-zinc-600">kWh</span></div>
        ...
        <div class="space-y-4 bg-black rounded-lg p-10 flex items-center justify-between flex-col w-full h-full">
            ...
            <div><span class="text-[7vw] leading-none outline-text">{{parseFloat(state.solarData.socialContribution.co2ReductionByYear / 1000).toFixed(2)}}</span><span class="text-[2vw] text-zinc-600">tons</span></div>
        ...
        <div class="space-y-4 bg-black rounded-lg p-10 flex items-center justify-between flex-col w-full h-full">
            ...
            <div><span class="text-[7vw] leading-none outline-text">{{parseFloat(state.solarData.realKpi.cumulativeEnergy / 1000).toFixed(2)}}</span><span class="text-[2vw] text-zinc-600">mWh</span></div>
            <div class="text-[2vw] text-slate-400">Udbytte i alt</div>
        </div>
        <div class="bg-black rounded-lg p-10 flex flex-col justify-between relative overflow-hidden col-span-2">
